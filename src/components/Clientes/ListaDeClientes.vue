    ...
    <v-card>
      ...
      <v-toolbar color="light-blue" dark>
        ...
        <v-flex xs12 v-show="estaBuscando">
          ...
          <v-text-field v-model="busqueda" ref="inputBusqueda" hide-details label="Escriba parte del nombre"
                        solo-inverted single-line></v-text-field>
        </v-flex>
      ...
      <v-list two-line subheader>
        <v-list-tile v-for="(cliente, indice) in clientes" :key="indice" avatar>
          <v-list-tile-content>
            ...
            <v-list-tile-sub-title>{{ cliente.NumeroTelefono }}</v-list-tile-sub-title>
          </v-list-tile-content>
          <v-list-tile-action>
            <v-btn target="_blank" :href="'tel:' + cliente.NumeroTelefono" icon ripple>
